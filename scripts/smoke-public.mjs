...
const manifest=JSON.parse(readFileSync('.sites-runtime/upload/manifest.json'));
...
for(const [path,item] of Object.entries(manifest)){
 ...
 const extension=extname(path).slice(1);assert.equal(createHash('sha256').update(body.toString('base64')+extension).digest('hex').slice(0,32),item.hash,path);
}
...
const result={checkedAt:new Date().toISOString(),origin,pages:4,matchedAssets:Object.keys(manifest).length,sharingDisabled:true,crossOriginWritesRejected:true};
